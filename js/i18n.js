(function () {
  ...
  var jpTextMap = {
    ...
    '다시 재생': 'もう一度再生',
    '맨 위로': 'トップに戻る',
    'CSCAM MLP 1000-5A 레이저 잉그레이빙 머신 외관': 'CSCAM MLP 1000-5A レーザーエングレービングマシン外観',
    '연속파 레이저 딩플 형성 단면': '連続波レーザーディンプル形成断面',
    '펄스 레이저 딩플 형성 단면': 'パルスレーザーディンプル形成断面',
    '펄스 폭 스펙트럼 다이어그램: 나노·피코·팜토초': 'パルス幅スペクトル図:ナノ・ピコ・フェムト秒',
    '일반 패턴 상세': '一般パターン詳細',
    '마이크로 패턴 상세': 'マイクロパターン詳細',
    '가공 디테일 대화면': '加工ディテール拡大画面',
    '패턴가공 1': 'パターン加工1',
    '패턴가공 1-2': 'パターン加工1-2',
    '패턴가공 2': 'パターン加工2',
    '패턴가공 3': 'パターン加工3',
    '패턴가공 4': 'パターン加工4',
    '패턴가공 5': 'パターン加工5',
    'MLP 1000-5A 다중 배치 설치 전경': 'MLP 1000-5A 複数台設置全景',
    'MLP 1000-5A 장비 외관': 'MLP 1000-5A 設備外観',
    '5축 로터리 테이블 상세': '5軸ロータリーテーブル詳細',
    '가공 소재 클로즈업': '加工材料クローズアップ',
    '국내 D사 실제 가동 현장 사진': '国内D社実際稼働現場写真',
    '유럽 D사 실제 가동 현장 사진': '欧州D社実際稼働現場写真',
    '레이저 스캐너와': 'レーザースキャナーと',
    '레이저 스캐너와 CNC 제어를 완전 결합한 5축 대면적 고정밀 마이크로 패턴 가공기 MLP 1000-5A. 타이어 사이드 몰드 각인 최적화, 단차·이음매 없는 3D 마이크로 텍스처링. — CSCAM 기계사업부': 'レーザースキャナーとCNC制御を完全に結合した5軸大面積高精度マイクロパターン加工機MLP 1000-5A。タイヤサイドモールド刻印に最適化、段差・継ぎ目のない3Dマイクロテクスチャリング。— CSCAM機械事業部',
    '레이저 스캐너 + CNC 결합 5축 대면적 마이크로 패턴 가공기. 타이어 몰드 각인 특화.': 'レーザースキャナー+CNC結合の5軸大面積マイクロパターン加工機。タイヤモールド刻印に特化。',
    '가 결합된': 'が結合された',
    '대면적 고정밀 가공 시스템': '大面積高精度加工システム',
    '웹 브라우저가 비디오 태그를 지원하지 않습니다.': 'お使いのブラウザはビデオタグに対応していません。',
    'CSCAM은': 'CSCAMは',
    '기술로 열 변형 없이 소재 표면에 정밀 미세 패턴을 구현합니다. 펄스 폭(Pulse Duration)에 따른 Nano·Pico·Femto초 레이저의 가공 특성을 최적화하여': '技術で熱変形なく材料表面に精密微細パターンを実現します。パルス幅(Pulse Duration)によるナノ・ピコ・フェムト秒レーザーの加工特性を最適化し',
    '을 동시에 달성합니다.': 'を同時に達成します。',
    '에너지를 연속적으로 조사하여 소재를': 'エネルギーを連続的に照射し材料を',
    '하는 방식입니다. 열 영향부(HAZ)가 형성되어 미세 패터닝 시 열 변형 및 버(Burr)가 발생할 수 있습니다.': 'する方式です。熱影響部(HAZ)が形成され、微細パターニング時に熱変形及びバリ(Burr)が発生する場合があります。',
    '극소 시간 동안 첨단 에너지를 집속시켜 소재를': '極めて短い時間に先端エネルギーを集束させ材料を',
    '시키는 방식입니다. 주변 소재로의 열 전도를 차단하여 왜곡 없는 완벽한 에지를 도출합니다.': 'させる方式です。周辺材料への熱伝導を遮断し、歪みのない完璧なエッジを実現します。',
    '소재 제거율(Removal Rate)이 높아 가공 속도가 빠름. 미세한 열영향부(HAZ)가 존재하나 대용량 제거 및': '材料除去率(Removal Rate)が高く加工速度が速い。微細な熱影響部(HAZ)は存在するが大容量除去及び',
    '에 적합.': 'に適合。',
    '가공 효율과 정밀도의 균형점. 열 영향을 대폭 억제하면서 정밀한 표면 질감 구현 및': '加工効率と精度のバランス点。熱影響を大幅に抑制しつつ精密な表面質感を実現及び',
    '에 적용.': 'に適用。',
    '열 전달 시간보다 짧은 초단펄스로 "Cold Ablation" 구현. 열 왜곡 및 Burr가 전혀 없는': '熱伝達時間より短い超短パルスで「コールドアブレーション」を実現。熱歪み及びバリが全くない',
    '완성.': 'を完成。',
    'CSCAM의 제어 시스템은 나노초(Nano) 및 펨토초(Femto) 펄스 폭 조절을 모두 지원하며, 본 장비(MLP 1000-5A)는': 'CSCAMの制御システムはナノ秒(Nano)及びフェムト秒(Femto)パルス幅調節の両方に対応しており、本設備(MLP 1000-5A)は',
    '와 5축 로터리 제어를 하드웨어 레이어에서 정밀히 정합하여 열영향부(HAZ) 왜곡이 거의 없는 완벽한 에지를 도출해 냅니다.': 'と5軸ロータリー制御をハードウェアレイヤーで精密に整合させ、熱影響部(HAZ)歪みがほぼない完璧なエッジを実現します。',
    '브랜드 시인성 50% 향상 솔루션': 'ブランド視認性50%向上ソリューション',
    '기존의 일반 가공 패턴은 금속 경계면의 난반사로 인해 로고나 정보 코드의 시인성이 크게 떨어집니다. CSCAM의 마이크로 패턴 텍스처링 공정은 빛의 각도를 인위적으로 차단하여 칠흑 같은 입체 구조를 생성함으로써, 일반 가공 대비': '従来の一般加工パターンは金属境界面の乱反射によりロゴや情報コードの視認性が大幅に低下します。CSCAMのマイクロパターンテクスチャリング工程は光の角度を人為的に遮断し漆黒のような立体構造を生成することで、一般加工に比べ',
    '을 제공합니다.': 'を提供します。',
    '국내외 글로벌 고객사에 적용되어 장비 품질 및 내구성이 확인되었습니다.': '国内外のグローバル顧客に適用され、設備品質及び耐久性が確認されています。',
    '(유럽, 한국, 러시아, 중국, 필란드 등 수 백조 이상)': '(欧州、韓国、ロシア、中国、フィンランドなど数百台以上)',
    '다운로드': 'ダウンロード',
    '기계사업부': '機械事業部',
